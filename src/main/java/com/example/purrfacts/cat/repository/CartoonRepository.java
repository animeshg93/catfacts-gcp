...
public class CartoonRepository {
  ...
  private final RowMapper<Cartoon> cartoonRowMapper =
      (rs, rowNum) -> {
  ...
  public int save(Cartoon cartoon) {
    ...
    return jdbcTemplate.update(
        sql, cartoon.getName(), cartoon.getYear(), cartoon.getAbbreviation());
  }

  public Cartoon getByAbb(String abbreviation) {
    String sql = "SELECT * FROM cartoons WHERE abbreviation = ?";
    return jdbcTemplate.query(sql, cartoonRowMapper, abbreviation).stream()
        .findFirst()
        .orElse(null);
  }
